...
#así defino qué es cada cosa,, guardo la información de estas webs en R. Después le puedo preguntar por cada cosa poniendo '...'
```
```{r}
#por ejemplo para protein pongo
protein
```
...
class(pfam01) #qué clase de objeto es. (es una lista)
length(pfam01) #longitud de la lista, cuántos elementos hay
names(pfam01) #cuando están nombrados, sirve para después acceder a cada elemento por su nombre
pfam01[['count']] #número
pfam01[['next']] #web, nueva URL para descargar 20 entradas
pfam01[['previous']] #nada, nulo
pfam01[['results']] #hay más cosas dentro, pero no sale nada al compilar
...
```{r}
library('kableExtra')
pfam02 <- fromJSON(pfam01[['next']]) #URL con los siguientes 20 resultados (next)
kable(pfam02[['results']][['metadata']]) %>% kable_styling() #kable sirve para hacer tablas elegantes
```
...
```{r}
pfam_1y2 <- rbind_pages(list(pfam01[['results']][['metadata']],
...
taxones: https://www.ebi.ac.uk/interpro/api/taxonomy
proteínas: https://www.ebi.ac.uk/interpro/api/protein
todo junto: https://www.ebi.ac.uk/interpro/api/protein/taxonomy (o con el orden inverso, da igual)
Para saber taxones sin proteína asignada restamos los resultados de /taxonomy a los de /protein/taxonomy:
2678841 - 1291419 = 1387422 taxones de UniProtKB que NO tienen proteína asignada
CON CÓDIGO:
```{r}
taxTot <- fromJSON(paste0(API, 'taxonomy'))[['taxa']][['uniprot']]
taxCon <- fromJSON(paste0(API, 'taxonomy/protein'))[['taxa']][['uniprot']]
taxSin <- taxTot - taxCon
taxSin
```

esrtuctura terciaria: structure, para estructuras tridimensionales
junto: https://www.ebi.ac.uk/interpro/api/protein/structure
proteínas que tienen estructura tridimensional definida:
reviewed: 37334
unreviewed: 30128
uniprot (aprox la suma): 67874
proteínas en general:
"reviewed": 571702,
"unreviewed": 201794580,
"uniprot": 202150435

PROPORCIÓN: dividir con estructura/sin estructura · 100
REVIEWED: 6,53%
UNREVIEWED: 0,015%
UNIPROT: 0,033%
...
```{r}
cocodrilo <- fromJSON(paste0(API, 'protein/entry/pfam/PF01530/taxonomy/uniprot/8493'))
names(cocodrilo)
cocodrilo[['proteins']]
```
...
# Si no existe, la creamos:
   ...
   dim(equinodermata) #nos dice la dimensión de esto
}
```
