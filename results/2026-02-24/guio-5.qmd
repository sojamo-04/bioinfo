...
- URL: 
```{r}
curl_download(URLencode('https://www.ebi.ac.uk/ena/portal/api/search?result=sequence&query=tax_tree(32281) AND description="ADH"&fields=accession,description,tax_id,base_count,first_public'), destfile = 'z3.1.txt')
```
...
query <- 'query=tax_tree(32281) AND description="ADH"&'
fields <- 'fields=accession,description,tax_id,base_count,scientific_name,first_public&'
limit <- 'limit=0'
...
- para que lea la tabla:
```{r}
adh <- read.table('z3.2.txt', sep = '\t', quote = "", header = TRUE)
```
después, ponemos dim(adh) en la consola de R y nos pone cuantas secuencias hay (cuantos componentes de la tabla): "[1] 559   3". dim es para saber la dimensión de los datos
(con quote le estamos diciendo que no hay nada que esté ultilizando el abrir comillas, que no le haga caso).

2. ¿Qué longitudes tienen? 
> summary(adh$base_count)
   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
  109.0   711.0   846.0   891.7   884.0 13087.0 
  
3. ¿De cuántas especies diferentes son estas secuencias?
137

4. ¿Cuándo se publicaron las más recientes?
En el caso de mi búsqueda todas son de la fecha 2023-06-14.
...
```{r}
plat <-platelmintos
plat$last_updated <- as.Date(plat$last_updated,
                             '%Y-%m-%d')
